    ...
    try:
        ...
        if ret == 0 and len(positions) > 0:
            for _, pos in positions.iterrows():
                ...
                today_pl = safe_float(pos.get("today_pl_val", 0))
                data["positions"].append({
                    "symbol": symbol,
                    "quantity": qty,
                    "avg_cost": safe_float(pos.get("cost_price", 0)),
                    ...
                    "unrealized_pnl_pct": safe_float(pos.get("pl_ratio", 0)),
                    "day_change": today_pl / qty if qty > 0 else 0,
                })
    finally:
        trd_ctx.close()
